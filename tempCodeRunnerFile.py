...
def upload_image():
    try:
        ...
        file_path = f"/tmp/{uuid.uuid4()}.png"  # 임시 경로에 저장
        file.save(file_path)
        ...
        ocr_text = extract_ocr_text(ocr_data)  # OCR 데이터에서 텍스트만 추출
        if not ocr_text:
        ...
        print("[INFO] Data saved to MongoDB.")

        return jsonify({"status": "success", "message": "업로드 및 분석 완료.", "data": final_data})
...
def analyze_schedule_with_openai(ocr_text, student_name, student_id):
    """
    GPT-4 API를 사용하여 시간표 분석 및 JSON 데이터 반환
    """
    prompt = f"""
    ...
    """
    try:
        print("[INFO] OpenAI GPT API 호출 중...")
        ...
        # print(f"[INFO] OpenAI 응답 원문: {response_text}")
